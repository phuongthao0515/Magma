...
class ShowUIImageProcessor(BaseImageProcessor):
    ...
    def _vis_uigraph(self, uigraph_assign, image_size, patch_size, image):
        ...
        # Resize image to match the upscaled uigraph dimensions if they don't match
        # The error "boolean index did not match indexed array" happens when image and mask sizes differ
        # upscaled_uigraph_assign is derived from the grid, which might be slightly larger than the original image due to padding/resizing logic
        if image.shape[0] != upscaled_uigraph_assign.shape[0] or image.shape[1] != upscaled_uigraph_assign.shape[1]:
            # Resize image to match the mask
            import cv2
            # cv2.resize expects (width, height)
            image = cv2.resize(image, (upscaled_uigraph_assign.shape[1], upscaled_uigraph_assign.shape[0]), interpolation=cv2.INTER_NEAREST)
